(() => {
  ...
  let lastResult = null;
  // 演示放大：把侧栏条框等比放大到全屏竖屏，让会场看清（比例不变，纯 transform: scale）
  ...
  function syncZoomButton() {
    ...
    if (btn) {
      ...
      btn.title = panelZoomed ? "还原大小" : "放大到全屏（演示用）";
    }
  }
  ...
  function buildPanelHeader(titleText) {
    ...
    zoomBtn.title = panelZoomed ? "还原大小" : "放大到全屏（演示用）";
  ...
  function handleStreamDone(data) {
    ...
    lastResult = data;
    const n = (data.claims || []).length;
    // 挂件标签前置真/假（产品要"确定真/假"）；派生逻辑复用 TNRender.binaryLean，与结论卡一致
  ...
  function appendEventCard(ev) {
    ...
    if (card) {
      ...
      // （产品/用户要求：别在用户读某张过程卡时把视图拽到底部。请勿"修回"自动滚到底。）
    }
  }
