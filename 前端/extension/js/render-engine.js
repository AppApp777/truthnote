...
(function (global) {
  ...
  registerTemplate("evidence_list", function (event) {
    ...
    // 真联网取证徽章：把"我们真的去搜了，不是问大模型基模知识"摆到台面上（产品叙事补强）
    if (data.live) {
      ...
      if (crawled != null && pages.length) {
        ...
        pages.forEach(p => {
          ...
          // （标题是真的、唯一，搜索首条就是这篇真报道）——保证点下去一定弹出真网页，绝不 404 报错。
  ...
  // 渲染 verdict 概率分布：按概率降序，最大项加粗 + 着色；横条从 0 长出（演示动效）
  ...
  // 产品叙事核心：不是"问大模型一句真假"，而是把每条证据和声明的关系逐条标出来
  ...
  // 产品叙事核心：判定不是模型说了算，是固定优先级规则匹配出来的（可审计、可复现）
  ...
  // 产品不接受"无法核实"当主态——主推确定真/假案例，这里把真/假倾向摆到结论卡最前。
  ...
  function renderFinalResult(data) {
    ...
    // 二元真/假前置 banner（结论卡最前，产品要"确定真/假"，少说"无法核实"）
